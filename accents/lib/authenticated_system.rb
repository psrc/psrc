  ...
  protected
    ...
    def redirect_back_or_default(default)
      respond_to do |format|
        format.html { session[:return_to] ? redirect_to_url(session[:return_to]) : redirect_to(default) }
        format.xml { head :ok, :location => session[:return_to] || default }
        format.yaml { head :ok, :location => session[:return_to] || default }
      end
