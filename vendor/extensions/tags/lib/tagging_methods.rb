TaggingMethods = Proc.new do
  ...
  def tag_list
   ...
   def self.tagged_with(*tag_list)
     ...
     sql << "HAVING COUNT(taggings.meta_tag_id) = #{tag_list.size}" # Restricts results to those that have *all* the specified tags
   ...
   def self.parse_tags(tags)
     ...
     tags.map { |tag| tag.strip.squeeze(" ") }.flatten.reject(&:blank?).map(&:downcase).uniq
   end
