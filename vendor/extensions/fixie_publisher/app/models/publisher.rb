...
require 'open3'
class Publisher
  TABLES = %w( events event_options users page_parts pages layouts snippets assets banners banner_placements page_attachments menu_choices meta_tags taggings )
  def self.publish!
  ...
  def self.publish_job
    ...
    `rsync -avz --delete /sites/psrc-staging/shared/assets/ /sites/psrc-production/shared/assets/`
  end
