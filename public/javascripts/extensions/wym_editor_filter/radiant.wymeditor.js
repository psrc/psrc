...
function boot_wym(elem) {
  ...
  jQuery(elem).wymeditor({
    ...
    /*
    classesItems: [
          {'name': 'align-left', 'title': 'PARA: Align Left', 'expr': 'p'},
          {'name': 'align-center', 'title': 'PARA: Align Center', 'expr': 'p'},
          {'name': 'align-right', 'title': 'PARA: Align Right', 'expr': 'p'},
          {'name': 'float-left', 'title': 'PARA: hidden', 'expr': 'p'},
   ...
   dialogImageHtml:  "<body class='wym_dialog wym_dialog_image'"
               ...
               + "<form>"
               + "<fieldset>"
               + "<p class='small'>Need to upload a new image or asset?  <a href='/admin/assets/new' target='blank'>Click here to go to the New Asset Form</a> (opens in new tab in parent window), then come back and refresh this sub-window."
               + "<input type='hidden' class='wym_dialog_type' value='"
               ...
               + "jQuery('legend').after(jQuery('<div><label>Select an asset to populate the fields below</label></div><div id=\"assets\"></div>'));"
               + "var assets = window.opener.assets;"
               + "for(i=0;i<assets.length;i++){"
               + "var iconFileName = assets[i].asset_file_name.replace(/\\./,'_icon.');"
               + "var filePath = '/assets/' + assets[i].id + '/';"
               + "jQuery('#assets').append('<dl><dt><img rel=\"' + filePath + assets[i].asset_file_name + '\" src=\"'+ filePath + iconFileName +'\" /></dt><dd>' + assets[i].title + '</dd></dl>');"
               + "}"
               + "jQuery('dl').click(function(){"
               + "jQuery('.wym_src').val(jQuery(this).find('img').attr('rel'));"
               ...
               + "</form>"
               + "</body>",
    dialogFeatures:    "menubar=yes,titlebar=no,toolbar=yes,resizable=yes"
