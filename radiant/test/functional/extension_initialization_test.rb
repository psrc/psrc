  ...
  private
    ...
    def assert_admin_tabs(more_tabs = [])
      ...
      Radiant::AdminUI.tabs.each do |tab|
        assert !tab_names.include?(tab.name), "duplicate tab #{tab.name.inspect}"
        tab_names << tab.name
      end
      more_tabs.each do |tab_name|
        assert Radiant::AdminUI.tabs.any? {|tab| tab.name == tab_name }
      end
    end
    ...
    def assert_view_paths(expected_messages = {}, message = nil)
      expected_messages.reverse_merge!(
      ...
      [:index, :override, :routing].each do |action|
        get action
        assert_match expected_messages[action], @response.body, message
      end
    end
end
