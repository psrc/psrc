...
class StandardTagsTest < Test::Unit::TestCase
  ...
  def test_tag_date_now
    assert_renders Time.now.strftime("%A, %B %d, %Y"), '<r:date for="now" />'
  end
  def test_tag_date_other_attributes
  ...
  def test_tag_snippet_with_global_page_propagation
    assert_renders "#{@page.title} " * @page.children.count, '<r:snippet name="global_page_cascade" />'
  end
  def test_tag_snippet_with_recursion_global_page_propagation
    @page = Page.find(6)
