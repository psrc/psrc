class BehaviorGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      m.template 'model.rb',      File.join('app/behaviors', class_path, "#{file_name}_behavior.rb")
      m.template 'unit_test.rb',  File.join('test/unit/behaviors', class_path, "#{file_name}_behavior_test.rb")
    end
  end
